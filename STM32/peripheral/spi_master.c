...
static void spi_master_init()
{
	...
	// PA5  SCK SPI1: Alternate function output push-pull, 50MHz
	// PA6 MISO SPI1: Floating input
	// PA7 MOSI SPI1: Alternate function output push-pull, 50MHz
	...
	// CPOL = 0, CPHA = 0
	SPI->CR1 = SPI_CR1_SSM_Msk | SPI_CR1_SSI_Msk | (1 << SPI_CR1_BR_Pos) | SPI_CR1_MSTR_Msk;
...
static void spi_write(const uint8_t *p, unsigned int len)
{
	while (len--)
		spi_transfer(*p++);
}

static void spi_transfer_buf(uint8_t *p, unsigned int len)
{
	for (; len--; p++)
		*p = spi_transfer(*p);
}
